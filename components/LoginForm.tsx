// components/LoginForm.tsx
"use client";

import { useState, FormEvent, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import ErrorMessage from './ErrorMessage';
import LoadingSpinner from './LoadingSpinner';
import { signInWithEmail, signInWithGoogle, getRedirectPath, getSession, getUserRole } from '@/lib/supabase/auth';

// Error messages - centralized for consistent messaging across the application
const ERROR_MESSAGES = {
  // Authentication errors
  INVALID_CREDENTIALS: 'Email o contraseña incorrectos',
  NETWORK_ERROR: 'Error de conexión. Por favor, verifica tu internet',
  NO_ROLE: 'Tu cuenta no tiene un rol asignado. Contacta al administrador',
  GOOGLE_CANCELLED: 'Inicio de sesión con Google cancelado',
  UNKNOWN_ERROR: 'Ocurrió un error inesperado. Intenta nuevamente',
  
  // Validation errors
  EMAIL_REQUIRED: 'El email es requerido',
  EMAIL_INVALID: 'Por favor ingresa un email válido',
  PASSWORD_REQUIRED: 'La contraseña es requerida',
  
  // Additional specific errors
  EMAIL_NOT_CONFIRMED: 'Por favor confirma tu email antes de iniciar sesión',
  USER_NOT_FOUND: 'No existe una cuenta con este email',
  TOO_MANY_REQUESTS: 'Demasiados intentos. Por favor intenta más tarde',
  POPUP_BLOCKED: 'Por favor permite las ventanas emergentes para continuar',
};

interface LoginFormState {
  email: string;
  password: string;
  isLoading: boolean;
  error: string | null;
  validationErrors: {
    email?: string;
    password?: string;
  };
}

export default function LoginForm() {
  const router = useRouter();
  const [formState, setFormState] = useState<LoginFormState>({
    email: '',
    password: '',
    isLoading: false,
    error: null,
    validationErrors: {},
  });

  // Check for existing session on mount (handles OAuth callback)
  useEffect(() => {
    const checkSession = async () => {
      const session = await getSession();
      
      if (session?.user) {
        // User is already authenticated, redirect based on role
        const role = await getUserRole(session.user.id);
        
        if (role) {
          // Map database role to auth role and redirect
          const authRole = mapDatabaseRoleToAuthRole(role);
          const redirectPath = getRedirectPath(authRole);
          router.push(redirectPath);
        } else {
          // No role assigned, show error and redirect to default page
          setFormState(prev => ({
            ...prev,
            error: ERROR_MESSAGES.NO_ROLE,
          }));
          // Redirect to default page after showing error
          setTimeout(() => {
            router.push('/');
          }, 3000);
        }
      }
    };

    checkSession();
  }, [router]);

  // Helper function to map database role to auth role
  const mapDatabaseRoleToAuthRole = (dbRole: string): 'restaurant' | 'admin' | 'client' | 'delivery' => {
    switch (dbRole) {
      case 'delivery_agent':
        return 'delivery';
      case 'client':
        return 'client';
      case 'restaurant':
        return 'restaurant';
      case 'admin':
        return 'admin';
      default:
        return 'client'; // Default fallback
    }
  };

  // Email validation
  const validateEmail = (email: string): string | undefined => {
    if (!email.trim()) {
      return ERROR_MESSAGES.EMAIL_REQUIRED;
    }
    
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
      return ERROR_MESSAGES.EMAIL_INVALID;
    }
    
    return undefined;
  };

  // Password validation
  const validatePassword = (password: string): string | undefined => {
    if (!password) {
      return ERROR_MESSAGES.PASSWORD_REQUIRED;
    }
    
    return undefined;
  };

  // Validate entire form
  const validateForm = (): boolean => {
    const emailError = validateEmail(formState.email);
    const passwordError = validatePassword(formState.password);

    setFormState(prev => ({
      ...prev,
      validationErrors: {
        email: emailError,
        password: passwordError,
      },
    }));

    return !emailError && !passwordError;
  };

  // Handle email/password login
  const handleEmailLogin = async () => {
    try {
      setFormState(prev => ({ ...prev, isLoading: true, error: null }));

      const result = await signInWithEmail(formState.email, formState.password);

      if (!result.success) {
        // Display the specific error message from the auth service
        setFormState(prev => ({
          ...prev,
          isLoading: false,
          error: result.error || ERROR_MESSAGES.INVALID_CREDENTIALS,
        }));
        return;
      }

      // Success - redirect based on role
      if (result.role) {
        const redirectPath = getRedirectPath(result.role);
        router.push(redirectPath);
      } else {
        // No role assigned - show error and redirect to home
        setFormState(prev => ({
          ...prev,
          isLoading: false,
          error: ERROR_MESSAGES.NO_ROLE,
        }));
        // Redirect to default page after showing error
        setTimeout(() => {
          router.push('/');
        }, 3000);
      }
    } catch (error) {
      // Catch any unexpected errors (network issues, etc.)
      const errorMessage = error instanceof Error && error.message.includes('fetch')
        ? ERROR_MESSAGES.NETWORK_ERROR
        : ERROR_MESSAGES.UNKNOWN_ERROR;
      
      setFormState(prev => ({
        ...prev,
        isLoading: false,
        error: errorMessage,
      }));
    }
  };

  // Handle Google OAuth login
  const handleGoogleLogin = async () => {
    try {
      setFormState(prev => ({ ...prev, isLoading: true, error: null }));

      const result = await signInWithGoogle();

      if (!result.success) {
        // Display the specific error message from the auth service
        setFormState(prev => ({
          ...prev,
          isLoading: false,
          error: result.error || ERROR_MESSAGES.GOOGLE_CANCELLED,
        }));
        return;
      }

      // OAuth flow initiated successfully - user will be redirected to Google
      // After successful auth, they'll return to the callback URL
      // Keep loading state active during redirect
    } catch (error) {
      // Catch any unexpected errors (network issues, popup blocked, etc.)
      const errorMessage = error instanceof Error && error.message.includes('fetch')
        ? ERROR_MESSAGES.NETWORK_ERROR
        : ERROR_MESSAGES.UNKNOWN_ERROR;
      
      setFormState(prev => ({
        ...prev,
        isLoading: false,
        error: errorMessage,
      }));
    }
  };

  // Handle form submission
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    // Clear previous errors
    setFormState(prev => ({
      ...prev,
      error: null,
      validationErrors: {},
    }));

    // Validate form
    if (!validateForm()) {
      return;
    }

    // Proceed with email login
    await handleEmailLogin();
  };

  return (
    <div className="w-full max-w-md mx-auto px-4 py-8">
      <div className="bg-white rounded-lg shadow-md p-6 sm:p-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 text-center mb-6">
          Iniciar Sesión
        </h1>

        {/* Global error message with ARIA live region for screen reader announcements */}
        {formState.error && (
          <div 
            className="mb-4" 
            role="alert" 
            aria-live="polite"
            aria-atomic="true"
          >
            <div className="message message-error">
              <svg 
                className="message-icon" 
                fill="currentColor" 
                viewBox="0 0 20 20"
                aria-hidden="true"
              >
                <path
                  fillRule="evenodd"
                  d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
                  clipRule="evenodd"
                />
              </svg>
              <div className="message-content">
                <span>{formState.error}</span>
              </div>
            </div>
          </div>
        )}

        <form onSubmit={handleSubmit} noValidate>
          {/* Email field */}
          <div className="mb-4">
            <label
              htmlFor="email"
              className="block text-sm font-medium text-gray-700 mb-1"
            >
              Email
              <span className="text-red-500 ml-1" aria-hidden="true">*</span>
            </label>
            <input
              type="email"
              id="email"
              value={formState.email}
              onChange={(e) =>
                setFormState(prev => ({
                  ...prev,
                  email: e.target.value,
                  validationErrors: { ...prev.validationErrors, email: undefined },
                }))
              }
              disabled={formState.isLoading}
              autoComplete="email"
              className={`
                mt-1 block w-full px-3 py-2 border rounded-md shadow-sm 
                transition-all duration-200 bg-white text-gray-900 
                placeholder-gray-400 disabled:bg-gray-50 disabled:text-gray-500 
                disabled:cursor-not-allowed min-h-[44px]
                ${
                  formState.validationErrors.email
                    ? 'border-red-500 focus:ring-2 focus:ring-red-500 focus:border-red-500'
                    : 'border-gray-300 focus:ring-2 focus:ring-[#e4007c] focus:border-[#e4007c]'
                }
              `}
              aria-required="true"
              aria-invalid={!!formState.validationErrors.email}
              aria-describedby={formState.validationErrors.email ? 'email-error' : undefined}
            />
            {formState.validationErrors.email && (
              <ErrorMessage
                message={formState.validationErrors.email}
                id="email-error"
                className="mt-1"
              />
            )}
          </div>

          {/* Password field */}
          <div className="mb-6">
            <label
              htmlFor="password"
              className="block text-sm font-medium text-gray-700 mb-1"
            >
              Contraseña
              <span className="text-red-500 ml-1" aria-hidden="true">*</span>
            </label>
            <input
              type="password"
              id="password"
              value={formState.password}
              onChange={(e) =>
                setFormState(prev => ({
                  ...prev,
                  password: e.target.value,
                  validationErrors: { ...prev.validationErrors, password: undefined },
                }))
              }
              disabled={formState.isLoading}
              autoComplete="current-password"
              className={`
                mt-1 block w-full px-3 py-2 border rounded-md shadow-sm 
                transition-all duration-200 bg-white text-gray-900 
                placeholder-gray-400 disabled:bg-gray-50 disabled:text-gray-500 
                disabled:cursor-not-allowed min-h-[44px]
                ${
                  formState.validationErrors.password
                    ? 'border-red-500 focus:ring-2 focus:ring-red-500 focus:border-red-500'
                    : 'border-gray-300 focus:ring-2 focus:ring-[#e4007c] focus:border-[#e4007c]'
                }
              `}
              aria-required="true"
              aria-invalid={!!formState.validationErrors.password}
              aria-describedby={formState.validationErrors.password ? 'password-error' : undefined}
            />
            {formState.validationErrors.password && (
              <ErrorMessage
                message={formState.validationErrors.password}
                id="password-error"
                className="mt-1"
              />
            )}
          </div>

          {/* Submit button */}
          <button
            type="submit"
            disabled={formState.isLoading}
            className={`
              w-full flex items-center justify-center px-4 py-3 
              border border-transparent rounded-md shadow-sm text-base 
              font-medium text-white bg-[#e4007c] min-h-[44px]
              transition-all duration-200
              ${
                formState.isLoading
                  ? 'opacity-50 cursor-not-allowed'
                  : 'hover:bg-[#c0006a] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#e4007c]'
              }
            `}
            aria-busy={formState.isLoading}
          >
            {formState.isLoading ? (
              <>
                <LoadingSpinner size="sm" className="mr-2" />
                <span>Iniciando sesión...</span>
              </>
            ) : (
              'Iniciar Sesión'
            )}
          </button>

          {/* Divider */}
          <div className="relative my-6">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-gray-300"></div>
            </div>
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-white text-gray-500">o</span>
            </div>
          </div>

          {/* Google login button */}
          <button
            type="button"
            onClick={handleGoogleLogin}
            disabled={formState.isLoading}
            className={`
              w-full flex items-center justify-center px-4 py-3 
              border border-gray-300 rounded-md shadow-sm text-base 
              font-medium text-gray-700 bg-white min-h-[44px]
              transition-all duration-200
              ${
                formState.isLoading
                  ? 'opacity-50 cursor-not-allowed'
                  : 'hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#e4007c]'
              }
            `}
            aria-busy={formState.isLoading}
          >
            <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" aria-hidden="true">
              <path
                fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="#34A853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="#FBBC05"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="#EA4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            Continuar con Google
          </button>
        </form>
      </div>
    </div>
  );
}
