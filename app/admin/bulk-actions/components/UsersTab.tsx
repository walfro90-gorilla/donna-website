...
import { Search, FlaskConical, Trash2, ToggleRight, RefreshCw, ShoppingBag } from 'lucide-react';
import {
  fetchTestUsers,
  previewDeleteUsers,
  deleteUsersBulk,
...
const ROLE_OPTIONS = [
  ...
  { value: 'delivery_agent', label: 'Repartidor' },
];
...
export default function UsersTab() {
  ...
  const [filterNoOrders, setFilterNoOrders] = useState(false);
  ...
  function handleSearch() {
    startTransition(async () => {
      ...
      const { data, error } = await fetchTestUsers({
        isTest,
        emailContains: filterEmail || undefined,
        role: filterRole || undefined,
        noRealOrders: filterNoOrders || undefined,
  ...
  function toggleAll() {
    setSelected((prev) => {
      const next = prev.size === users.length
        ? new Set<string>()
  ...
  function handlePreview() {
    ...
    startTransition(async () => {
      const { data, error } = await previewDeleteUsers(Array.from(selected));
      if (error) { toast.error(error); return; }
      setPreview(data);
    });
  }
  ...
  function handleMarkAsTest(isTest: boolean) {
    if (!selected.size) return;
    startTransition(async () => {
      const { error } = await markAsTest('user', Array.from(selected), isTest);
      if (error) { toast.error(error); return; }
      toast.success(`${selected.size} usuario(s) marcado(s) como ${isTest ? 'prueba' : 'real'}`);
      handleSearch();
    });
  }
  ...
  async function handleDelete() {
    ...
    try {
      const result = await deleteUsersBulk(Array.from(selected));
      if (result.error) { toast.error(result.error); return; }

      const msg = result.skipped.length > 0
        ? `${result.deleted} usuario(s) eliminado(s). ${result.skipped.length} omitido(s): ${result.skipped.map((s) => s.reason).join(', ')}.`
        : `${result.deleted} usuario(s) eliminado(s) correctamente.`;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 cursor-pointer">
            <input
              type="checkbox"
              checked={filterNoOrders}
              onChange={(e) => setFilterNoOrders(e.target.checked)}
      ...
      {/* Preview */}
      <ImpactPreview preview={preview} entityType="users" selectedCount={selected.size} />
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Rol</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Pedidos</th>
                  ...
                  {users.map((user) => (
                    <tr
                      ...
                      className={`hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${
                        selected.has(user.id) ? 'bg-blue-50 dark:bg-blue-950/20' : ''
                      }`}
                    >
      ...
      <ConfirmModal
        isOpen={showConfirm}
        title={`Eliminar ${selected.size} usuario(s)`}
        description="Esta acción es irreversible. Se eliminarán el usuario, sus pedidos, perfil y datos financieros. También se eliminará su acceso (auth)."
...
function RoleBadge({ role }: { role: string }) {
  const map: Record<string, string> = {
    ...
    delivery_agent: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300',
