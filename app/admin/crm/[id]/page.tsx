...
import { ArrowLeft, CheckCircle, Bot, BotOff, Loader2, ShoppingBag } from 'lucide-react';
...
import CreateOrderPanel from '@/components/admin/crm/CreateOrderPanel';
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-background shadow-sm">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-[#e4007c] bg-pink-50 dark:bg-pink-950/20 text-[#e4007c] hover:bg-pink-100 dark:hover:bg-pink-950/40 transition-colors font-medium"
        ...
        <div className="w-64 flex-shrink-0 hidden xl:flex flex-col border-l border-border bg-background overflow-y-auto p-3 space-y-3">
          {conversation && (
            <ContactCard
              ...
              onCreateOrder={() => setShowOrderPanel(true)}
            />
          )}
        </div>
      </div>

      {/* Order creation slide-over */}
      {showOrderPanel && conversation && adminUser && (
        <CreateOrderPanel
          conversation={conversation}
